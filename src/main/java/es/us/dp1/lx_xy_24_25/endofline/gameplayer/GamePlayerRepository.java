...
//    @Query("SELECT gp FROM GamePlayer gp, Friendship f WHERE gp.game.id = :gameId AND f.friendState = 'ACCEPTED' AND (f.sender.id = :userId AND f.receiver.id = gp.user.id) OR (f.receiver.id = :userId AND f.sender.id = gp.user.id)")
    @Query("""
    ...
    """)
    Optional<GamePlayer> findFriendInGame(Integer userId, Integer gameId);
}
